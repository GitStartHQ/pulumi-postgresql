PROJECT_NAME := postgresql Package
include build/common.mk

PACK             := postgresql
PACKDIR          := sdk
PROJECT          := github.com/murcul/pulumi-${PACK}
NODE_MODULE_NAME := @murcul/pulumi-${PACK}
...
build::
	go install -ldflags "-X github.com/murcul/pulumi-${PACK}/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${TFGEN}
	go install -ldflags "-X github.com/murcul/pulumi-${PACK}/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${PROVIDER}
...
install::
	GOBIN=$(PULUMI_BIN) go install -ldflags "-X github.com/murcul/pulumi-${PACK}/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${PROVIDER}
